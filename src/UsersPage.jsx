...
import { Check, Pause, Play, Plus, ShieldCheck, Users } from 'lucide-react';
...
export function UsersPage({run,busy}) {
  ...
  return <><PageTitle title="Acessos e permissões" subtitle="Usuários dos Açores"><button className="primary-button" disabled={busy} onClick={() => setModal({type:'new'})}><Plus />Novo acesso</button></PageTitle>
    ...
    {modal && <Modal title={modal.type === 'new' ? 'Novo acesso' : modal.type === 'active' ? `${modal.user.active ? 'Suspender' : 'Reativar'} acesso?` : 'Alterar perfil de acesso'} onClose={() => setModal(null)}>
