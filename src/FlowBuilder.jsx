...
const palette = [
  ...
  ['trigger-keyword', 'Palavra-chave', GitBranch, 'green', 'Inicia ao reconhecer um termo'],
  ['trigger-contact', 'Novo contato', UserRound, 'green', 'Inicia no primeiro contato'],
  ['message', 'Enviar mensagem', MessageCircle, 'purple', 'Responde com um texto aprovado'],
  ['message-media', 'Enviar mídia', MessageCircle, 'purple', 'Envia imagem, documento, áudio ou vídeo'],
  ['question', 'Perguntar dados', UserRound, 'blue', 'Nome, espécie, idade ou outro dado'],
  ['input-selection', 'Perguntar seleção', UserRound, 'blue', 'Oferece opções para o cliente escolher'],
  ['input-date', 'Perguntar data', Clock3, 'blue', 'Coleta uma data válida'],
  ['condition', 'Condição', GitBranch, 'amber', 'Divide o fluxo em Sim e Não'],
  ['switch', 'Switch', GitBranch, 'amber', 'Cria múltiplos caminhos'],
  ['check-variable', 'Verificar variável', GitBranch, 'amber', 'Compara uma variável do contato'],
  ['check-hours', 'Verificar horário', Clock3, 'amber', 'Verifica o expediente'],
  ...
  ['queue', 'Colocar na fila', Send, 'teal', 'Envia para um setor de atendimento'],
  ['pause', 'Pausar automação', Clock3, 'teal', 'Pausa até um atendente retomar'],
  ['ai', 'Fallback IA', Bot, 'violet', 'Usa a IA se nenhum bloco combinar'],
  ['ai-agent', 'Agente IA', Bot, 'violet', 'Responde com contexto e regras aprovadas'],
  ['ai-intent', 'Classificar intenção', Bot, 'violet', 'Classifica a intenção da mensagem'],
  ['wait', 'Aguardar tempo', Clock3, 'pink', 'Espera antes de continuar'],
  ['flow-other', 'Ir para outro fluxo', ArrowRight, 'pink', 'Continua em outro fluxo'],
  ['http', 'HTTP Request', ArrowRight, 'cyan', 'Chama uma integração externa'],
  ['contact', 'Atualizar contato', UserRound, 'cyan', 'Salva dados do cliente'],
  ['tag', 'Adicionar tag', GitBranch, 'cyan', 'Organiza o contato por tags'],
  ['finish', 'Finalizar', X, 'gray', 'Encerra o fluxo'],
];

const defaults = {
  'trigger-message': ['Mensagem recebida', 'Inicia quando uma mensagem chegar.'],
  'trigger-keyword': ['Palavra-chave', 'Inicia quando a mensagem contiver:'],
  'trigger-contact': ['Novo contato', 'Inicia no primeiro contato do cliente.'],
  message: ['Enviar mensagem', 'Quando receber uma mensagem, responder com:'],
  'message-media': ['Enviar mídia', 'Enviar imagem, documento, áudio ou vídeo configurado.'],
  question: ['Perguntar dados do pet', 'Qual é o nome, espécie e idade do seu pet?'],
  condition: ['Condição', 'Se a mensagem contiver:'],
  switch: ['Switch', 'Escolher um caminho conforme o valor:'],
  'check-variable': ['Verificar variável', 'Se a variável atender à regra:'],
  'check-hours': ['Verificar horário', 'Se estiver dentro do horário de atendimento.'],
  ...
  queue: ['Colocar na fila', 'Enviar para a fila de atendimento:'],
  pause: ['Pausar automação', 'Pausar até um atendente retomar.'],
  ai: ['Fallback IA', 'Se nenhum bloco combinar, responder usando a IA aprovada.'],
  'ai-agent': ['Agente IA', 'Responder com contexto e regras aprovadas, sem inventar informações.'],
  'ai-intent': ['Classificar intenção', 'Classificar a intenção da mensagem.'],
  wait: ['Aguardar tempo', 'Aguardar 5 segundos antes de responder.'],
  'flow-other': ['Ir para outro fluxo', 'Continuar a execução em outro fluxo.'],
  http: ['HTTP Request', 'Executar uma integração HTTP configurada.'],
  contact: ['Atualizar contato', 'Salvar os dados coletados no contato.'],
  tag: ['Adicionar tag', 'Adicionar uma tag ao contato.'],
