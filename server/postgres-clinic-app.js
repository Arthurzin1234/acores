...
export async function createPostgresClinicApp({ rootDir, dataDir, authDir }) {
  ...
  security.routes(app);
  app.use('/api', (req, res, next) => { if (req.path === '/legal' || req.path.startsWith('/auth/')) return next(); if (!req.user) return res.status(401).json({ error: 'Entre para continuar.' }); next(); });
  ...
  const snapshot = async () => ({ stats: await db.getStats(), clients: await db.listClients(), tickets: await db.listTickets(), notifications: await db.listNotifications(), whatsapp: whatsapp.snapshot(), petshopName: (await clinic.snapshot()).settings.name, company, ...(await clinic.snapshot()), ai: { ...(await aiConfig.snapshot()), runtime: ai.snapshot() } });
  ...
  app.patch('/api/clients/:id', async (req, res) => { try { const client = await db.updateClient(Number(req.params.id), req.body); if (!client) return res.status(404).json({ error: 'Cliente nao encontrado.' }); res.json(client); } catch { res.status(400).json({ error: 'Não foi possível concluir a operação. Confira os dados.' }); } });
  app.get('/api/tickets', async (_req, res) => res.json(await db.listTickets()));
  app.get('/api/tickets/:id/messages', async (req, res) => { const ticket = await db.getTicket(Number(req.params.id)); if (!ticket) return res.status(404).json({ error: 'Chamado nao encontrado.' }); res.json({ ticket, messages: await db.listMessages(ticket.id) }); });
  app.patch('/api/tickets/:id', async (req, res) => { const ticket = await db.updateTicket(Number(req.params.id), req.body); if (!ticket) return res.status(404).json({ error: 'Chamado nao encontrado.' }); res.json(ticket); });
  app.post('/api/tickets/:id/messages', async (req, res) => { const ticket = await db.getTicket(Number(req.params.id)); const body = String(req.body.body || '').trim(); if (!ticket) return res.status(404).json({ error: 'Chamado nao encontrado.' }); if (!body) return res.status(400).json({ error: 'Mensagem vazia.' }); await db.updateTicket(ticket.id, { ai_paused: true, status: 'em_atendimento', human_required: true }); const message = await db.addMessage(ticket.id, { direction: 'outbound', author: req.user.email, body }); res.status(201).json({ message, delivery: { delivered: false, reason: 'Mensagem registrada no painel.' } }); });
  ...
  app.get('/api/whatsapp/status', (_req, res) => res.json(whatsapp.snapshot()));
  app.post('/api/whatsapp/start', async (_req, res) => res.json(await whatsapp.start()));
  app.post('/api/whatsapp/sync', async (_req, res) => res.json(whatsapp.snapshot()));
  app.post('/api/whatsapp/relink', async (req, res) => { if (req.body?.confirmed !== true || !whatsapp.snapshot().requiresNewQr) return res.status(400).json({ error: 'Confirme a substituição da sessão inválida.' }); res.json(await whatsapp.start({ newSession: true })); });
  app.get('/api/operations/status', async (_req, res) => res.json({ checkedAt: new Date().toISOString(), process: true, database: true, ai: ai.snapshot(), whatsapp: whatsapp.snapshot(), queue: whatsapp.snapshot().queue, alerts: [] }));
  app.post('/api/operations/check', async (_req, res) => { await ai.health(); res.json({ checkedAt: new Date().toISOString(), process: true, database: true, ai: ai.snapshot(), whatsapp: whatsapp.snapshot(), queue: whatsapp.snapshot().queue, alerts: [] }); });
